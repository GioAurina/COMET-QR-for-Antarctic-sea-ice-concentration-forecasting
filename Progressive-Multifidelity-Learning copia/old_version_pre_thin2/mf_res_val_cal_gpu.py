...
class DataLoader_Ice:
    ...
    def preprocess_data(self, mask_land_, mask_ice_, data_, data_mean_month_, data_mean_week_, x_, y_, thickness_data_, sst_data_):
        ...
        # Crop to bounding box around active region
        print("\n" + "-"*70)
        print("SPATIAL CROPPING TO ACTIVE REGION BOUNDING BOX")
        print("-"*70)
        
        # Define active region first (ice probability > 0)
        ice_presence = np.zeros_like(mask_ice, dtype=float)
        for year_data in data:
            ice_presence += (year_data > 0.1).sum(axis=0)
        ice_probability = ice_presence / (len(data) * data[0].shape[0])
        active_mask = (ice_probability > 0.05) & mask_ice
...
class LowFidelityDataPrep:
    ...
    def prepare_sensors(self, y_true_data, y_dmd_pred, dmd_years, n_sensors=128, seed=0):
        ...
        Returns:
            sensor_residuals: (n_years, n_days, n_sensors) - DMD residuals at sensor locations
        ...
        """
        print(f"\nPREPARING LEVEL 3: SENSORS (n={n_sensors}, sampling DMD residuals)")
        print("-"*70)
        ...
        print(f"  Data shape: {sensor_residuals.shape} (years × days × sensors)")
        print(f"  Residual statistics at sensors:")
        print(f"    Mean: {sensor_residuals.mean():.6f}")
        print(f"    Std:  {sensor_residuals.std():.6f}")
        print(f"    Min:  {sensor_residuals.min():.6f}")
        print(f"    Max:  {sensor_residuals.max():.6f}")
        
        return sensor_residuals, sensor_mask, sensor_idxs, n_sensors
...
class DataScaler:
    ...
    def scale_all_levels(self, x1_train, x1_val, x1_test,
        ...
        print("Level 1 (Ice Thickness POD): SKIPPED (raw POD projection, no scaling)")
        med1, iqr1 = None, None
        
        print("Level 2 (SST POD): SKIPPED (raw POD projection, no scaling)")
        med2, iqr2 = None, None
        ...
        scalers = {
            'level_1': {'median': med1, 'iqr': iqr1, 'method': 'POD_projection'},
            'level_2': {'median': med2, 'iqr': iqr2, 'method': 'POD_projection'},
...
class ConformalCalibration:
    ...
    def calibrate_conditional(self, val_loader, residual_scaler):
        """Calibrate Q-scores for each scenario
        
        CRITICAL: For residual-based approach, we must:
        1. Denormalize predicted residuals (scaled_pred * std + mean)
        2. Add DMD baseline to get final SIC predictions
        3. Clip to [0, 1] physical range
        4. Compute conformity scores on physical SIC values
        ...
        for name, mask_cfg in self.scenarios.items():
            ...
            with torch.no_grad():
                for batch in tqdm(val_loader, desc=f"Calibrating {name}"):
                    ...
                    low_norm = preds[:, :, 0, :]  # Lower quantile (0.05)
                    high_norm = preds[:, :, 2, :]  # Upper quantile (0.95)
                    y_norm = target
                    
                    # Denormalize residuals
                    low_res = low_norm * res_std + res_mean
                    high_res = high_norm * res_std + res_mean
                    y_res = y_norm * res_std + res_mean
                    
                    # NOTE: In validation, we don't have DMD baseline in the batch
                    # So we compute scores on residuals directly (which is correct for residual prediction)
                    # The Q-score will be applied to residuals, then DMD baseline added at test time
                    
                    # Compute conformity scores on denormalized residuals
                    y_flat = y_res.flatten()
                    low_flat = low_res.flatten()
                    high_flat = high_res.flatten()
        ...
        print("  Note: Q-scores computed on residuals, will be applied before DMD baseline addition")
        return q_dict
...
def main():
    ...
    # Set split configuration to match updated requirements (24/6/1)
    ...
    q_scores = calibrator.calibrate_conditional(val_loader, residual_scaler)
