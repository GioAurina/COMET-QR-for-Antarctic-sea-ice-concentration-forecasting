...
class MultiFidelityDataset(Dataset):
    ...
    def __init__(self, features_by_level, targets, device, identifiers=None, sequential=False):
        super().__init__()
        self.device = device
    ...
    def _build_dataset(self, features_by_levels, targets, identifiers=None):
        ...
        for i, target in enumerate(targets):
            # Loop on all the possible masks
            for mask in self.mask_manager.ids_to_mask.values():
