...
class TransformerBlock(nn.Module):
    ...
    # OLD IMPLEMENTATION WITHOUT SKIP CONNECTIONS (COMMENTED OUT)
    ...
    def forward(self, x, attn_mask=None):
        """Forward pass with skip connections (residual connections).
        
        Standard Transformer architecture:
        1. Multi-head attention + residual + normalization
        2. Feed-forward network + residual + normalization
        ...
        x = x + self.dropout_attn(attn_x)  # Skip connection (residual)
        x = self.norm_attn(x)
        
        # Feed-forward block with skip connection
        ffn_x = self.ffn(x)
        x = x + self.dropout_ffn(ffn_x)  # Skip connection (residual)
        output = self.norm_ffn(x)
...
class MultifidelityTransformer(nn.Module):
    ...
    def _build_transformer_blocks(self, n_heads: int, n_transformer_blocks: int, dropout: float) -> nn.ModuleList:
        ...
        for _ in range(n_transformer_blocks):
            transformer_block = TransformerBlock(
                ...
                ffn_hidden_dim=4 * self.embedding_dim,  # Standard Transformer: 4x embedding dimension
                dropout=dropout
            )
    ...
    def _build_decoder(self, dropout: float) -> nn.ModuleDict:
        #Build the decoder that combines embeddings and parameters to produce the output.
        ...
        return nn.ModuleDict({
            ...
            'projector': nn.Linear(self.embedding_dim, self.output_dim)
        })
    
    """def _build_decoder(self, dropout: float) -> nn.Module:
        
        # ============================================================
        # MLP DECODER (Optimized for sparse masks)
        # ============================================================
        
        gru_hidden_dim = 128  # Increased from 64 for more decoder capacity
        mlp_hidden_dim = 512  # Increased from 256 for large output dimensions
    ...
    def forward(self, x : Dict[str, torch.Tensor]) -> torch.Tensor:
            ...
            pooled_embedding = self._pool_embeddings(transformed, levels_mask) # (batch_size * seq_len, embedding_dim)
            ...
            # Pass through decoder GRU
            gru_output, _ = self.decoder['gru'](decoder_input) # (batch_size, seq_length, embedding_dim)
            
            # Apply MLP projection to get final output
            output = self.decoder['projector'](gru_output) # (batch_size, seq_length, output_dim)
